...
Before anything else, check whether `LEARNING_LOG.md` exists in the working directory.

- If it does **not** exist, create it now using the file header from the framework schema. Do not wait — create it silently as part of startup.

### Step 1: Detect mode

- If `$ARGUMENTS` contains "plan" or "reflect" explicitly, honor that directly — skip detection.
- If `LEARNING_LOG.md` is empty or was just created → Planning mode.
